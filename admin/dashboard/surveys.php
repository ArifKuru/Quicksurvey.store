<!DOCTYPE html>
<?php
$pageTitle="Dashboard | QuickSurvey";
?>
<html lang="en">
...
<div class="container mx-auto mt-3">
    <div class="container-fluid p-0" style="position: relative;">
       <h2 class="text-2xl font-semibold text-white" style="position: absolute;top: 15px;left: 20px">Strengthen your company with QuickSurvey!</h2>
        <i>
            <img src="https://filelist.tudelft.nl/_processed_/6/7/csm_TUD%20AI_MACHINE%20LEARNING_04c317144a.webp"
                 style="width: 100%; height: auto; border-radius: 25px;">
        </i>
        <button class="btn btn-primary bg-purple-600 hover:bg-purple-700 px-4 py-2 text-white"
                style="position: absolute; bottom: 75px; right: 20px;border-radius: 15px"
                data-bs-toggle="modal" data-bs-target="#createSurveyModalWithAi">
            <i class="fas fa-plus mr-2"></i>Create With AI
        </button>
        <button class="btn btn-secondary px-4 py-2 text-white"
                style="position: absolute; bottom: 15px; right: 20px;border-radius: 15px"
                data-bs-toggle="modal" data-bs-target="#createSurveyModal">
    ...
    <div class="flex justify-between items-center mb-4 mt-3">
        <h2 class="text-2xl font-semibold text-gray-800">Your Recent Surveys</h2>
...
<script>
    document.getElementById('sendToAiButton').addEventListener('click', function() {
        ...
        const url = `/api/sendRequestToAi.php?request=${encodeURIComponent(request)}`;
        ...
        fetch(url, {
            ...
            .then(data => {
                if (data.success) {
                    alert('Survey successfully created with Hash ID: ' + data.hash_id);
                    // Modal'ı kapat
